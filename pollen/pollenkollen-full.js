...
const getPollen = async () => {
  const result = await fetch(
    `${BASE_URL}/forecasts?region_id=${REGION_ID}&current=true`
  );
  const data = await result.json();

  const pollenData = data.items[0].levelSeries.sort(
    (a, b) => b.level - a.level
  );

  for (const pollen of pollenData) {
    if (!pollen.time.startsWith(today)) continue;

    const pollenLevel = pollen.level;
    const POLLEN_ID = pollen.pollenId;
